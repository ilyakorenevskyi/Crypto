...
public class MARS {
    ...
    public MARS(int[] key) {
        S0 = Arrays.copyOfRange(S, 0, 256);
        S1 = Arrays.copyOfRange(S, 0, 256);

        if (key.length < 4 || key.length > 14) {
            throw new IllegalArgumentException("Invalid key length must be >= 4 32-bit words and <= 14 32-bit words long");
        } else {
            keyExpansion(key, key.length);
        }
    }
    ...
    public MARS(byte[] key) {
        ...
        if (key.length / 4 < 4 || key.length / 4 > 14) {
            throw new IllegalArgumentException("Invalid key length must be >= 4 32-bit words and <= 14 32-bit words long");
        }
        S0 = Arrays.copyOfRange(S, 0, 256);
        S1 = Arrays.copyOfRange(S, 0, 256);

        int[] wKey = new int[key.length / 4];
        for (int i = 0, offset = 0; i < wKey.length && offset < key.length; i++, offset += 4) {
            wKey[i] = fromByteArray(Arrays.copyOfRange(key, offset, offset + 4));
        }
        keyExpansion(wKey, wKey.length);
    ...
    private int low8bits(int num) {
        return num & 0xFF;
    }

    private int low9bits(int num) {
        return num & 0x1FF;
    }

    // ROTATE LEFT METHOD
    private int rotateLeft(int val, int pas) {
        return (val << pas) | (val >>> (32 - pas));
    }

    //ROTATE RIGHT METHOD
    private int rotateRight(int val, int pas) {
        return (val >>> pas) | (val << (32 - pas));
    }

    private void keyExpansion(int[] key, int n) {
        int[] T = new int[n + 15];
        if (n >= 0) System.arraycopy(key, 0, T, 0, n);
        T[n] = n;
        for (int i = 1; i < 15; i++) {
            T[n + i] = 0;
        }
        ...
        for (int j = 0; j < 4; j++) {
            ...
            for (int i = 0; i < 15; i++) {
                T[i] = T[i] ^ (rotateLeft((T[Math.floorMod((i - 7), 15)] ^ T[Math.floorMod((i - 2), 15)]), 3)) ^ (4 * i + j);
            }
            //S-box Based Stirring of Key-Words
            for (int k = 0; k < 5; k++) {
                for (int i = 0; i < 15; i++) {
                    T[i] = rotateLeft((T[i] + S[T[Math.floorMod((i - 1), 15)] & 0x1FF]), 9);
                }
            }
            //Store Next 10 Key-Words in K
            for (int i = 0; i < 9; i++) {
                K[10 * j + i] = T[(4 * i) % 15];
            }
        }
        ...
        for (int i = 5; i < 36; i += 2) {
            int j = 0x3 & K[i];
            int w = K[i] & (0x3FFFFFFF << 2);
            int M = makeMask(w);

            int r = 0x1F & K[i - 1];
            int p = rotateLeft(B[j], r);
            K[i] = 2 ^ (p & M);
        }
    }
    ...
    public byte[] encrypt(byte[] data) {
        ...
        if (data.length % 8 != 0) {
            throw new IllegalArgumentException("Length should be divisible by 8");
        }
        int n = data.length / 4;//+ ((data.length % 4 != 0) ? 1 : 0);

        int[] words = new int[n];

        int[] res = new int[n];

        byte[] cipher = new byte[n * 4];


        for (int i = 0, offset = 0; i < n && offset < data.length; i++, offset += 4) {
            words[i] = fromByteArray(Arrays.copyOfRange(data, offset, offset + 4));
        }

        for (int j = 0; j < n; j += 4) {
            System.arraycopy(encryption(Arrays.copyOfRange(words, j, j + 4)), 0, res, j, 4);
        }

        for (int i = 0, offset = 0; i < n && offset < data.length; i++, offset += 4) {
            System.arraycopy(toByteArray(res[i]), 0, cipher, offset, 4);
        }
        return cipher;
    ...
    public byte[] decrypt(byte[] data) {
        ...
        for (int j = 0; j < n; j += 4) {
            System.arraycopy(decryption(Arrays.copyOfRange(words, j, j + 4)), 0, res, j, 4);
        }
    ...
    public int[] encryption(int[] data) {
        ...
        for (int i = 0; i < 8; i++) {
            B = (B ^ S0[low8bits(A)]) + S1[low8bits(rotateRight(A, 8))];
            C = C + S0[low8bits(rotateRight(A, 16))];
            D = D ^ S1[low8bits(rotateRight(A, 24))];
            A = (rotateRight(A, 24)) + ((i == 1 || i == 5) ? B : 0) + ((i == 0 || i == 4) ? C : 0);
            int tempA = A;
            A = B;
            B = C;
            C = D;
        ...
        int R, M, L;
        for (int i = 0; i < 16; i++) {
            R = rotateLeft((rotateLeft(A, 13) * K[2 * i + 5]), 10);
            M = rotateLeft((A + K[2 * i + 4]), rotateRight(R, 5) & 0x1F);
            L = rotateLeft((S[low9bits(M)] ^ rotateRight(R, 5) ^ R), R & 0x1F);
            if (i < 8) {
                B = B + L;
            } else {
                B = B ^ R;
            ...
            B = C;
            C = D;
            D = rotateLeft(tempA, 13);
        }

        //Backwards Mixing
        for (int i = 0; i < 8; i++) {
            A = A - ((i == 3 || i == 7) ? B : 0) - ((i == 2 || i == 6) ? D : 0);
            B = B ^ S1[low8bits(A)];
            C = C - S0[low8bits(rotateLeft(A, 8))];
            D = (D - S1[low8bits(rotateLeft(A, 16))]) ^ S0[low8bits(rotateLeft(A, 24))];
            int tempA = A;
            A = B;
            B = C;
            C = D;
            D = rotateLeft(tempA, 24);
        }
        A = A - K[36];
        B = B - K[37];
        C = C - K[38];
    ...
    public int[] decryption(int[] data) {
        ...
        for (int i = 7; i > -1; i--) {
            ...
            C = B;
            B = A;
            A = rotateRight(tempD, 24);
            D = D ^ S0[low8bits(rotateLeft(A, 24))];
            D = D + S1[low8bits(rotateLeft(A, 16))];
            C = C + S0[low8bits(rotateLeft(A, 8))];
            B = B ^ S1[low8bits(A)];
            A = A + ((i == 3 || i == 7) ? B : 0) + ((i == 2 || i == 6) ? D : 0);
        }
        ...
        for (int i = 7; i > -1; i--) {
            ...
            B = A;
            A = tempD;
            A = rotateLeft(A - ((i == 1 || i == 5) ? B : 0) - ((i == 0 || i == 4) ? C : 0), 24);
            D = D ^ S1[low8bits(rotateRight(A, 24))];
            C = C - S0[low8bits(rotateRight(A, 16))];
            B = B - S1[low8bits(rotateRight(A, 8))];
            B = (B ^ S0[low8bits(A)]);
    ...
    private int makeMask(int w) {
        String MBin = "0".repeat(32);
        char[] MBinChar = MBin.toCharArray();
        String w_bin = Integer.toBinaryString(w);
        w_bin = "0".repeat(32 - w_bin.length()) + w_bin;
        int ind = 0;
        while (true) {
            ind = w_bin.indexOf("1".repeat(10), ind);
            if (ind == -1) {
                break;
            }
            for (int i = 1; i < 9; i++) {
            ...
            if (ind != 0 && w_bin.charAt(ind) == w_bin.charAt(ind - 1)) {
                MBinChar[ind] = '1';
            }
            if (ind + 9 != 31 && w_bin.charAt(ind + 9) == w_bin.charAt(ind + 10)) {
                MBinChar[ind + 10] = '1';
            }
            ind++;
        }
        ind = 0;
        while (true) {
            ind = w_bin.indexOf("0".repeat(10), ind);
            if (ind == -1) {
                break;
            }
            for (int i = 1; i < 9; i++) {
        ...
        return Integer.parseInt(String.copyValueOf(MBinChar), 2);
    }
